...
export const LEVELS = {
  ...
  dificil: {
    count: 16,
    quotas: { flashcard: 4, classe: 3, mecanismo: 3, meia_vida: 2, interaction: 2, food: 1, protocol: 1 },
  },
}
...
export function buildLevelSession({ level = 'facil', pools, rng = Math.random }) {
  ...
  const picked = []
  for (const [cat, quota] of Object.entries(cfg.quotas)) {
    picked.push(...shuffle(byCategory[cat] || [], rng).slice(0, quota))
  }
