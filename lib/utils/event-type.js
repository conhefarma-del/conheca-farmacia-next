...
import { Monitor, MapPin, Shuffle } from 'lucide-react'

// Ícones Lucide (100% SVG) — substituem os emojis antigos (💻 📍 🔀)
const TYPE_ICONS = {
  online: Monitor,
  presencial: MapPin,
  hibrido: Shuffle,
 ...
 *
 * `icon` é um componente Lucide (ou null) — o chamador renderiza
 * `<Icon size={…} />`. Fallback: MapPin (presencial).
 *
 * @param {string} type
 * @param {(key: string, fallback?: string) => string} t
 * @returns {{icon: import('lucide-react').LucideIcon|null, label: string}}
 */
export function formatEventType(type, t) {
  const norm = (type || '').toLowerCase()
  const icon = TYPE_ICONS[norm] || MapPin
