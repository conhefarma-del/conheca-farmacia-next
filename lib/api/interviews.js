...
/**
 * Categorias de entrevistas (interview_categories, migração 155) — taxonomia
 * pública gerível no admin, ordenada por sort_order.
 */
export const getInterviewCategories = unstable_cache(
  async () => {
  const supabase = await createAnonClient()
  const { data, error } = await supabase
