 ...
 * Devolve { [drugId]: { slug, name } } — objeto plano porque o valor passa
 * por unstable_cache, que serializa o resultado (um Map não sobrevive à
 * desserialização e rebentava o RSC render com "object is not iterable").
 */
export const getFlashcardDrugMap = unstable_cache(
  async (drugIds) => {
    ...
    const map = {}
    for (const d of data || []) {
      map[d.id] = { slug: d.slug, name: d.name_pt }
    }
    return map
  },
