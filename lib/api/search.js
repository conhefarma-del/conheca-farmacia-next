 ...
 * @param {string} query
 * @param {'pt'|'en'} lang
 * @param {string} type - 'todos'|'artigos'|'eventos'|'lives'|'guias'|'protocolos'
 * @param {string} order - 'recente'|'antigo'
 */
export async function searchAllContent(query, lang = 'pt', type = 'todos', order = 'recente') {
  if (!query || !query.trim()) {
    return { articles: [], events: [], lives: [], guides: [], protocolos: [], total: 0 }
  }
  ...
  const [articlesRes, eventsRes, livesRes, guidesRes, protocolosRes] = await Promise.all(queries)
  ...
  return {
    ...
    guides,
    protocolos,
    total: articles.length + events.length + lives.length + guides.length + protocolos.length,
  }
}
