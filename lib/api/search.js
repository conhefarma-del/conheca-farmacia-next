 ...
 * @param {string} query
 * @param {'pt'|'en'} lang
 * @param {string} type - 'todos'|'artigos'|'eventos'|'lives'|'guias'|'protocolos'|'cientificos'|'autores'|'farmacos'|'interacoes'
 * @param {string} order - 'recente'|'antigo'
 */
export async function searchAllContent(query, lang = 'pt', type = 'todos', order = 'recente') {
  if (!query || !query.trim()) {
    return { articles: [], events: [], lives: [], guides: [], protocolos: [], cientificos: [], autores: [], farmacos: [], interacoes: [], total: 0 }
  }
  ...
  const [articlesRes, eventsRes, livesRes, guidesRes, protocolosRes, drugsRes, drugLookupRes, ffRes, foodRes, diseaseRes, pregRes, sciRes, authRes] = await Promise.all(queries)
