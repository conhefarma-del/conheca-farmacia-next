 ...
 * @param {string} query
 * @param {'pt'|'en'} lang
 * @param {string} type - 'todos'|'artigos'|'eventos'|'lives'|'guias'|'protocolos'|'farmacos'|'interacoes'
 * @param {string} order - 'recente'|'antigo'
 */
export async function searchAllContent(query, lang = 'pt', type = 'todos', order = 'recente') {
  if (!query || !query.trim()) {
    return { articles: [], events: [], lives: [], guides: [], protocolos: [], farmacos: [], interacoes: [], total: 0 }
  }
  ...
  const [articlesRes, eventsRes, livesRes, guidesRes, protocolosRes, drugsRes, drugLookupRes, ffRes, foodRes, diseaseRes, pregRes] = await Promise.all(queries)
  ...
  return {
    ...
    farmacos,
    interacoes,
    total: articles.length + events.length + lives.length + guides.length + protocolos.length + farmacos.length + interacoes.length,
  }
}
