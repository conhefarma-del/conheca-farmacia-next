...
export async function searchAllContent(query, lang = 'pt', type = 'todos', order = 'recente') {
  ...
  if (type === 'todos' || type === 'alvos') {
    queries.push(
      supabase
        ...
        .select('id, slug, name_pt, name_en, alias_pt, alias_en, type_pt, type_en, what_pt, what_en, sort_order')
        .or(`name_pt.ilike.${searchTerm},name_en.ilike.${searchTerm},alias_pt.ilike.${searchTerm},alias_en.ilike.${searchTerm},type_pt.ilike.${searchTerm},type_en.ilike.${searchTerm}`)
        .eq('status', 'published')
        .order('sort_order', { ascending: true })
    )
  } else {
  ...
  const alvos = (alvosRes.data || []).map((t) => ({
    id: t.id,
    slug: t.slug,
    title: isEn ? (t.name_en || t.name_pt) : t.name_pt,
    excerpt: isEn ? (t.type_en || t.type_pt) : (t.type_pt || t.type_en),
    date: null,
    image_url: null,
  }))
