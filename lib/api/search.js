 ...
 */
// Normaliza para busca sem acentos: NFD remove os diacríticos (á→a) e depois
// minúsculas. Aplicado ao termo E ao conteúdo em todos os filtros JS.
const norm = (s) =>
  String(s || '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
...
// Filtro por termo normalizado: verdadeiro se algum campo contém o termo.
const hit = (...fields) => {
...
// Termo normalizado atual (setado pela searchAllContent; usado por hit).
let _currentQuery = ''
...
export async function searchAllContent(query, lang = 'pt', type = 'todos', order = 'recente') {
  ...
  _currentQuery = norm(query)
  const ascending = order === 'antigo'
  const isEn = lang === 'en'

  const queries = []

  // Articles — dataset pequeno: busca o publicado e filtra em JS (permite
  // busca sem acentos via NFD, o .or() ilike do SQL é sensível a acentos).
  ...
  // Events — mesmo padrão: busca o publicado e filtra em JS.
  ...
  // Dataset pequeno: busca o publicado e filtra em JS (busca sem acentos).
  ...
  // entrevistas publicadas. Dataset pequeno: filtra em JS (busca sem acentos).
  ...
  // Dataset pequeno: filtra em JS (busca sem acentos).
  ...
  // Artigos/Eventos: filtra em JS (título/excerto, normalizado sem acentos).
  // Preserva os campos originais (article_id/event_id/events) para o branch
  // EN que faz lookup de arquivados e achata a data.
  let articles = (articlesRes.data || [])
    .filter((a) => hit(a.title, a.excerpt))
    .map((a) => ({ ...a, id: a.id || a.article_id }))
  let events = (eventsRes.data || [])
    .filter((e) => hit(e.title, e.excerpt))
    .map((e) => ({ ...e, id: e.id || e.event_id }))
  let farmacos = []
  let entrevistas = []
  let alvos = []

  // Guias: filtra em JS (nome/descrição PT+EN, normalizado sem acentos)
  const guides = (guidesRes.data || [])
    .filter((g) => hit(g.name_pt, g.name_en, g.description_pt, g.description_en, g.hero_subtitle_pt, g.hero_subtitle_en))
    .map((g) => ({
      id: g.id,
      slug: g.slug,
  ...
  const protocolos = (protocolosRes.data || [])
    .filter((p) => hit(p.title_pt, p.title_en, p.description_pt, p.description_en))
    .map((p) => ({
      id: p.id,
      slug: p.slug,
  ...
  farmacos = (drugsRes.data || [])
    .filter((d) => hit(d.name_pt, d.name_en, d.class_pt, d.class_en, (d.aliases || []).join(' ')))
    .map((d) => ({
      id: d.id,
      slug: d.slug,
  ...
  const cientificos = (sciRes.data || [])
    .filter((a) => hit(a.title, a.abstract))
    .map((a) => ({
      id: a.id,
      slug: a.slug,
      title: a.title,
  ...
  const autores = (authRes.data || [])
    .filter((a) => hit(a.name, a.institution, a.department, a.role))
    .map((a) => ({
      id: a.id,
      slug: a.slug,
      title: a.name,
  ...
  entrevistas = (interviewsRes.data || [])
    .filter((i) => {
      const person = i.interviewee && typeof i.interviewee === 'object'
        ...
        : ''
      return hit(i.title, i.excerpt, person)
    })
    .map((i) => ({
      id: i.id,
      slug: i.slug,
  ...
  const entrevistados = (peopleRes.data || [])
    .filter((p) => hit(p.name, p.role, p.bio))
    .map((p) => ({
      id: p.id,
      slug: p.slug,
      title: p.name,
  ...
  const flashcards = (flashDecksRes.data || [])
    .filter((d) => hit(d.name_pt, d.name_en, d.description_pt, d.description_en))
    .map((d) => ({
      id: d.id,
      slug: d.slug,
  ...
  alvos = (alvosRes.data || [])
    .filter((t) => hit(t.name_pt, t.name_en, t.what_is_pt, t.what_is_en, t.role_pt, t.role_en, (t.aliases || []).join(' ')))
    .map((t) => ({
      id: t.id,
      slug: t.slug,
