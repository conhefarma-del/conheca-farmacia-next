...
/**
 * Author pages — resolve um autor pelo slug (slugify do nome) e devolve
 * o seu perfil consolidado + todos os artigos publicados onde aparece.
 *
 * Os autores vivem no JSONB `authors` de cada artigo (não há tabela de
 * autores), por isso o identificador estável é o slug do nome. Nota: dois
 * autores distintos com nomes iguais partilhariam o mesmo slug — para o
 * volume atual (artigos científicos) é aceitável, e a fusão é feita aqui
 * de forma determinística (primeira ocorrência fornece os campos-base).
 *
 * @param {string} authorSlug - slugify(nome do autor)
...
export async function getScientificAuthor(authorSlug, lang = 'pt') {
  const articles = await getScientificArticles(lang)

  const matches = []
  let author = null
  for (const article of articles) {
    const found = (article.authors || []).find((au) => slugify(au.name) === authorSlug)
    if (found) {
  ...
  return { author: { ...author, slug: authorSlug }, articles: matches }
}
