...
import { findRelatedScientificArticles } from '@/lib/api/related-scientific'

export { findRelatedScientificArticles }
 ...
 */
export const getScientificCategories = unstable_cache(
  async (lang = 'pt') => {
  ...
  ['api', 'scientific', 'categories'],
  { revalidate: 3600, tags: ['scientific'] }
)
...
export const getScientificArticles = unstable_cache(
  async (lang = 'pt') => {
  const supabase = await createAnonClient()
  const { data, error } = await supabase
 ...
 */
export const getScientificArticleBySlug = unstable_cache(
  async (slug, lang = 'pt') => {
  const supabase = await createAnonClient()
 ...
 */
export const getScientificAuthors = unstable_cache(
  async (lang = 'pt') => {
  const supabase = await createAnonClient()
 ...
 */
export const getScientificAuthor = unstable_cache(
  async (authorSlug, lang = 'pt') => {
  const supabase = await createAnonClient()
