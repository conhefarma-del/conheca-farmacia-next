...
export async function getArticles(lang = 'pt') {
  ...
  return bases
    ...
    .filter((merged) => {
      // On EN, only keep articles that have a translation (or are at least
      // a fallback-pt that we still want to show). For now we keep fallbacks
      // to preserve the current "always show" behaviour; the banner
      // component handles UX. We do NOT filter by translation existence
      // here, only by `status = published` which is already applied.
      return true
    })
}
