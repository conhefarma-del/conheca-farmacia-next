...
export function mergeEntity(baseRow, translationRow, lang) {
  ...
  // Reconstruct nested author/host objects so that camelCase merges
  // (authorRole, authorBio, hostRole, hostBio) are reflected in the
  // nested objects used by the frontend components.
  if (merged.author) {
  ...
  if (merged.host) {
    ...
    if (merged.hostRole !== undefined) merged.host.role = merged.hostRole
    if (merged.hostBio !== undefined) merged.host.bio = merged.hostBio
  }
