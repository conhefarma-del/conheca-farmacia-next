...
export async function getEventsWithInscriptionCounts(lang = 'pt') {
  ...
  const bases = (data || []).filter((e) => e.is_archived === false).map(normalizeEventWithCount)
...
/**
 * Call the `get_event_inscription_count(slug)` RPC and return the count.
 * Returns 0 on RPC error so the page can still render.
 */
export async function getEventInscriptionCount(slug) {
  if (!slug) return 0
  const supabase = await createClient()
  const { data, error } = await supabase.rpc('get_event_inscription_count', {
    event_slug: slug,
  })
  if (error) {
    console.error('getEventInscriptionCount rpc error:', error)
    return 0
  }
