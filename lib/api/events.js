...
const EVENT_COLUMNS = 'id, slug, title, excerpt, image_url, category, category_label, date, time, end_time, location, type, capacity, hosts, status, featured_langs, registration_link'
 ...
 */
export async function getFeaturedEvents(limit = 2, lang = 'pt') {
  const supabase = await createClient()
  const { data, error } = await supabase.rpc('get_events_with_inscription_counts')
  if (error) throw error
