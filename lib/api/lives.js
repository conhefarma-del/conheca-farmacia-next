...
const LIVE_COLUMNS = 'id, slug, title, excerpt, category, category_label, image_url, date, time, end_time, platform, access_link, meeting_id, password, materials, status, featured_langs, host_name, host_role, host_organization, view_count, access_count, download_count'
...
export async function getFeaturedLives(limit = 2, lang = 'pt') {
  const supabase = await createClient()
  const { data, error } = await supabase
    .from('lives')
    .select(LIVE_COLUMNS)
