...
export const getQuizPools = unstable_cache(
  async () => {
    ...
    // Drug classes para perguntas "A que classe pertence X?"
    const drugClasses = (drugClassesRes.data || []).map((c) => ({
      id: c.id,
      name: c.name_pt,
      atcPrefix: c.atc_prefix || '',
    }))

    // Mapear drug_id -> class info (para o builder drug_class)
    const classInfoByDrugId = new Map()
    for (const drug of drugsRes.data || []) {
      if (drug.class_id) {
        const cls = (drugClassesRes.data || []).find((c) => c.id === drug.class_id)
        if (cls) {
          classInfoByDrugId.set(drug.id, {
            className: cls.name_pt,
            classPrefix: cls.atc_prefix || '',
          })
        }
      }
    }
    ...
    const pharm = (drugsRes.data || [])
      .map((d) => ({
        ...
        className: classInfoByDrugId.get(d.id)?.className || '',
        classPrefix: classInfoByDrugId.get(d.id)?.classPrefix || '',
      }))
      .filter((d) => d.class_pt || d.mechanism_pt || d.half_life_pt || d.className)
    ...
    return {
      ...
      counts: {
        ...
        drugClasses: drugClasses.length,
      },
    }
  },
  ['api', 'quiz', 'pools'],
...
export async function getQuizPoolCounts() {
  ...
  const tables = [
    ...
    { table: 'drug_classes', label: 'drugClasses' },
  ]
