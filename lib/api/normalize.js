...
export function normalizeEvent(row) {
  ...
  return {
    ...
    topic: row.topic,
    registrationEnabled: isSupabase
      ? row.registration_enabled !== false
      : row.registrationEnabled !== false,
  }
}

/**
 * Normaliza o campo `interviewee` (JSONB) para uma lista de pessoas.
 * Aceita o formato antigo (objeto único, migração 152) e o novo (array,
 * até 5 pessoas). Devolve sempre um array — vazio se não houver dados.
 */
function toInterviewees(raw) {
...
/**
 * Normalize interview from Supabase (snake_case) to frontend format (camelCase).
 * Módulo de Entrevistas (migration 152).
 */
export function normalizeInterview(row) {
  const isSupabase = 'category_label' in row || 'executive_summary' in row
  const interviewees = toInterviewees(row.interviewee)
  ...
  return {
    ...
    date: row.date,
    readTime: isSupabase ? row.read_time : row.readTime,
    videoDuration: isSupabase ? row.video_duration : row.videoDuration,
    thumbnailUrl: isSupabase ? row.thumbnail_url : row.thumbnailUrl,
    videoId: isSupabase ? row.video_id : row.videoId,
    audioUrl: isSupabase ? row.audio_url : row.audioUrl,
    executiveSummary: isSupabase ? row.executive_summary : row.executiveSummary,
    pullQuotes: row.pull_quotes || row.pullQuotes || [],
    qa: row.qa || [],
    content: row.content,
    references: isSupabase ? row.references_arr : row.references,
    related: row.related || [],
    ...
    viewCount: isSupabase ? row.view_count : row.viewCount,
    metaDescription: isSupabase ? row.meta_description : row.metaDescription,
  }
}
