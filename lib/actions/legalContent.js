...
export async function getPublicFAQData() {
  // Client anónimo: a página /faq é ISR e o client com cookies() em render
  // dinâmico faz a rota perder ISR (aviso do isr-guard) — as policies
  // anon_read_faq_* (Lição RLS migração 034) já expõem o público necessário.
 ...
 * Devolve { sections, lastUpdated } — lastUpdated = MAX(updated_at) das
 * secções visíveis (alimentado pelo trigger set_privacy_sections_updated_at),
 * usado no selo "Última atualização" da página pública.
 */
export async function getPublicPrivacyData() {
  // Client anónimo: ver getPublicFAQData (página ISR, policies anon_read_*).
  const supabase = await createAnonClient()

  try {
    const { data: sections, error } = await supabase
      .from('privacy_sections')
      .select('id, parent_id, anchor_slug, title_pt, title_en, content_pt, content_en, level, pending, sort_order, updated_at')
      .eq('is_archived', false)
      .order('sort_order', { ascending: true })

    if (error || !sections) return { sections: [], lastUpdated: null }
    ...
    // MAX(updated_at) das secções visíveis (string ISO comparável).
    // NB: com acumulador null, `s.updated_at > max` é sempre false na primeira
    // iteração (`x > null` === false) e o resultado seria null para sempre.
    const lastUpdated = sections.reduce(
      (max, s) => (!s.updated_at ? max : !max || s.updated_at > max ? s.updated_at : max),
      null
    )

    const structured = level1.map((section) => ({
      id: section.id,
    ...
    return { sections: structured, lastUpdated }
  } catch {
...
// ============================================================
//  PUBLIC DATA — Terms of Use
// ============================================================

/**
 * Retorna dados públicos dos termos de utilização: secções hierárquicas.
 * Secções pending são incluídas com flag pending=true (mostrar badge).
 * Devolve { sections, lastUpdated } — ver getPublicPrivacyData.
 */
export async function getPublicTermsData() {
  // Client anónimo: ver getPublicFAQData (página ISR, policies anon_read_*).
  const supabase = await createAnonClient()
...
export async function createPrivacySection({ parent_id, anchor_slug, title_pt, title_en, content_pt, content_en, level, pending, sort_order }) {
  ...
  const { supabase } = ctx
  try {
    // SEC-UMN-02 (auditoria "O Sentinela" #4): o conteúdo de privacidade é
    // renderizado com dangerouslySetInnerHTML na página pública. Sanitizar NA
    // GRAVAÇÃO (defense in depth — o render não faz escaping do markdown).
    const { data, error } = await supabase
      .from('privacy_sections')
      .insert({
        ...
        title_pt,
        title_en,
        content_pt: sanitizeHtml(content_pt || ''),
...
export async function updatePrivacySection(id, { anchor_slug, title_pt, title_en, content_pt, content_en, pending, sort_order }) {
  ...
  const { supabase } = ctx
  try {
    // SEC-UMN-02: sanitizar também no update (mesma fronteira de escrita que o create).
    const { error } = await supabase
      .from('privacy_sections')
      .update({
        anchor_slug,
...
// ============================================================
//  ADMIN — Terms Sections CRUD (mirror privacy_sections)
// ============================================================
...
export async function createTermsSection({ parent_id, anchor_slug, title_pt, title_en, content_pt, content_en, level, pending, sort_order }) {
  const ctx = await requireAdmin()
  if (!ctx) return { success: false, error: 'Não autorizado' }

  const { supabase } = ctx
  try {
    // SEC-UMN-02: mesmo padrão do createPrivacySection — o conteúdo é
    // renderizado com dangerouslySetInnerHTML na página pública, sanitizar
    // NA GRAVAÇÃO (defense in depth).
    const { data, error } = await supabase
      .from('terms_sections')
      .insert({
        parent_id: parent_id || null,
    ...
    if (error) return { success: false, error: error.message }
    revalidatePath('/[lang]/admin/conteudo-legal/termos')
    revalidatePath('/[lang]/termos')
    revalidatePath('/[lang]/terms')
    return { success: true, data }
  } catch {
    return { success: false, error: 'Erro ao criar secção' }
  }
...
export async function updateTermsSection(id, { anchor_slug, title_pt, title_en, content_pt, content_en, pending, sort_order }) {
  const ctx = await requireAdmin()
  if (!ctx) return { success: false, error: 'Não autorizado' }

  const { supabase } = ctx
  try {
    // SEC-UMN-02: sanitizar também no update (mesma fronteira de escrita).
    const { error } = await supabase
      .from('terms_sections')
      .update({
        anchor_slug,
...
export async function archiveTermsSection(id) {
  const ctx = await requireAdmin()
  if (!ctx) return { success: false, error: 'Não autorizado' }
...
export async function restoreTermsSection(id) {
  const ctx = await requireAdmin()
  if (!ctx || ctx.role !== 'superadmin') return { success: false, error: 'Não autorizado' }
