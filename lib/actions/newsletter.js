...
// Rate limit em memória por IP para sendWelcomeEmail. Este Server Action agora
// envia emails de verdade; sem limite era um vetor de spam de email (auditoria
// #9/#10). 5 pedidos/min./IP, fail-open. Espelha o padrão checkPollRate de
// lib/actions/inscription.js.
const WELCOME_RATE_MAX = 5
const WELCOME_RATE_WINDOW_MS = 60_000
const welcomeRate = new Map()

function checkWelcomeRate(ip) {
  const now = Date.now()
  const entry = welcomeRate.get(ip)
  if (!entry || entry.resetAt < now) {
    welcomeRate.set(ip, { count: 1, resetAt: now + WELCOME_RATE_WINDOW_MS })
    return true
  }
  if (entry.count >= WELCOME_RATE_MAX) return false
  entry.count += 1
  return true
}
