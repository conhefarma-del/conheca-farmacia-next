...
export async function startFriendQuiz(competitionId) {
  try {
    ...
    // Check if questions were already generated (partial success from previous attempt)
    const hasQuestions = players.some(p => p.questions && p.questions.length > 0)
    if (hasQuestions) {
      // Questions already generated, just update status
      ...
      return {
        ...
        timePerQuestion: comp.time_per_question || 30,
        streakBonus: comp.streak_bonus !== false,
        questions: players[0].questions.map(({ correctIndex, ...q }) => q),
      }
    }
