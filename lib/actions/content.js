...
// ============================================================
//  CATEGORIAS DE ENTREVISTAS — CRUD (migração 155)
// ============================================================

export async function createInterviewCategory(formData) {
  try {
    const ctx = await requireAdmin()
    if (!ctx) return { success: false, error: 'Sessão expirada. Faça login novamente.' }
    ...
    const name = String(formData.name || '').trim().slice(0, 120)
    ...
    await logAudit(supabase, user, 'CREATE', 'interview_categories', data.id, { name })
    revalidatePath(`/[lang]/admin/entrevistas`, 'page')
    revalidateTag('interviews')
    return { success: true, id: data.id }
  } catch {
...
export async function updateInterviewCategory(id, formData) {
  try {
    ...
    await logAudit(supabase, user, 'UPDATE', 'interview_categories', id, { name })
    revalidatePath(`/[lang]/admin/entrevistas`, 'page')
    revalidateTag('interviews')
    return { success: true }
  } catch {
...
export async function deleteInterviewCategory(id) {
  try {
    if (!id) return { success: false, error: 'ID da categoria é obrigatório.' }
    ...
    // As entrevistas guardam `category` = slug (não o id), por isso resolvemos
    // o slug antes de contar o uso
    ...
    // Não permitir eliminar categorias com entrevistas não arquivadas
    ...
    if (count > 0) {
      return { success: false, error: `Categoria em uso por ${count} entrevista(s). Reatribua ou arquive as entrevistas primeiro.` }
    }
    ...
    await logAudit(supabase, user, 'DELETE', 'interview_categories', id, { name: cat?.name })
    revalidatePath(`/[lang]/admin/entrevistas`, 'page')
    revalidateTag('interviews')
    return { success: true }
  } catch {
