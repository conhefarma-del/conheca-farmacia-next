...
// ============================================================
//  getNoteForItem — buscar a nota contínua de um item
// ============================================================
export async function getNoteForItem(savedItemId) {
  const { supabase, user } = await getUser()
  if (!user) return null
...
// ============================================================
export async function upsertNote(savedItemId, content) {
  const { supabase, user } = await getUser()
  if (!user) return { success: false, error: 'auth_required' }
  ...
  // Upsert: insere ou atualiza (uma nota por item)
  const { data, error } = await supabase
    .from('saved_item_notes')
    .upsert({
      saved_item_id: savedItemId,
