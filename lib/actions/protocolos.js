...
//  Público
// ============================================================
export const getPublicProtocolCategories = unstable_cache(
  async (lang = 'pt') => {
  ...
  ['api', 'protocolos', 'categories'],
  { revalidate: 3600, tags: ['protocolos'] }
)

// Forma dos itens consumida por ProtocolCard (listagem e relacionados)
function mapPublicProtocol(row, lang) {
  return {
    id: row.id,
    slug: row.slug,
    ...
    stepCount: row.clinical_protocol_steps?.[0]?.count || 0,
    categorySlug: row.clinical_protocol_categories.slug,
    categoryName: pickLang(row.clinical_protocol_categories, 'name', lang),
    categoryColor: row.clinical_protocol_categories.color,
  }
}

const PUBLIC_PROTOCOL_SELECT =
...
export const getPublicProtocols = unstable_cache(
  async (lang = 'pt') => {
  const supabase = await createAnonClient()
  const { data, error } = await supabase
    .from('clinical_protocols')
    .select(PUBLIC_PROTOCOL_SELECT)
    .eq('status', 'published')
    .eq('is_archived', false)
    .order('sort_order', { ascending: true })
  if (error) return []
  ...
  return (data || [])
    ...
    .map((p) => mapPublicProtocol(p, lang))
  },
  ['api', 'protocolos', 'list'],
  { revalidate: 3600, tags: ['protocolos'] }
)

// Protocolos da mesma categoria, excluindo o atual — secção "Protocolos relacionados"
export const getRelatedProtocols = unstable_cache(
  async (categorySlug, excludeSlug, lang = 'pt', limit = 3) => {
  const supabase = await createAnonClient()
  // Busca com margem e faz o fallback em JS: primeiramente protocolos da MESMA
  // categoria; se não chegarem ao limite (ex.: catálogo pequeno com 1 protocolo
  // por categoria), preenche com outros protocolos publicados. Sem isto a secção
  // 'Protocolos Relacionados' nunca aparecia (relacionados.length === 0).
  const { data, error } = await supabase
    .from('clinical_protocols')
    .select(PUBLIC_PROTOCOL_SELECT)
    .eq('status', 'published')
  ...
  const mapped = (data || [])
    .filter((p) => p.clinical_protocol_categories)
    .map((p) => ({
      ...mapPublicProtocol(p, lang),
      _categorySlug: p.clinical_protocol_categories.slug,
    }))

  const sameCategory = mapped.filter((p) => p._categorySlug === categorySlug)
  const others = mapped.filter((p) => p._categorySlug !== categorySlug)
  return [...sameCategory, ...others].slice(0, limit).map(({ _categorySlug, ...p }) => p)
  },
  ['api', 'protocolos', 'related'],
  { revalidate: 3600, tags: ['protocolos'] }
)

export const getPublicProtocolBySlug = unstable_cache(
  async (slug, lang = 'pt') => {
  const supabase = await createAnonClient()
