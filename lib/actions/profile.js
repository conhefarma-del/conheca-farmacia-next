...
/**
 * Fetch competition history for the current user.
 * Returns all sessions with competition name, score, accuracy, date.
 */
export async function getUserCompetitionHistory() {
  ...
  const { data: sessions } = await supabase
    .from("competition_sessions")
    .select(
      "id, total_score, correct_count, total_answered, max_streak, " +
      "finished_at, created_at, student_name, " +
      "competitions(name, access_code, questions_count)"
    )
    .eq("user_id", user.id)
    .order("created_at", { ascending: false })
    .limit(20);
  ...
  return sessions.map((s) => ({
    id: s.id,
    competitionName: s.competitions?.name || "Competição",
    accessCode: s.competitions?.access_code || "",
    totalScore: s.total_score || 0,
    correctCount: s.correct_count || 0,
    totalAnswered: s.total_answered || 0,
    accuracy:
      s.total_answered > 0
        ? Math.round((s.correct_count / s.total_answered) * 100)
        : 0,
