...
const inscriptionFormSchema = z.object({
  ...
  faixa_etaria: z.string().min(1, 'Faixa etária é obrigatória'),
  nivel_escolaridade: z.string().optional(),
  origem_evento: z.string().optional(),
  menor_consentimento: z.boolean().optional(),
  honeypot: z.string().optional(),
}).superRefine((val, ctx) => {
  // Política 2.1 — participantes menores de 18 anos exigem consentimento do responsável legal
  if (val.faixa_etaria === 'menor-18' && val.menor_consentimento !== true) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: 'O consentimento do responsável legal é obrigatório para participantes menores de idade.',
      path: ['menor_consentimento'],
    })
  }
})
...
// - Sucesso: { success: true, inscriptionId, shareCode, emailSent }
...
export async function apiCheckDuplicate(eventoId, email) {
  try {
    ...
    // SEC-ATH-05: rate-limit no "oráculo de enumeração" (auditoria "O Sentinela"
    // #3). Sem limite, um atacante podia iterar emails contra este endpoint e
    // descobrir quem está inscrito em que evento. Fail-open (devolve vazio) —
    // o check definitivo de duplicados acontece na RPC submit_inscription_safe.
    const headersList = await headers()
    const ip = getClientIp(headersList)
    if (!checkPollRate(ip)) {
      return { isDuplicate: false }
...
export async function apiSubmitInscription(form, eventoId, eventoSlug, honeypot = '', safeLang = 'pt') {
  ...
  const fnRes = await fetch(`${supabaseUrl}/functions/v1/validate-inscription`, {
    ...
    body: JSON.stringify({
      ...
      menor_consentimento: form.menor_consentimento || false,
  ...
  // RPC retornou 'ok': re-read do id recém-criado (e do share_code —
  // migração 253 — para o link de download do comprovativo PDF).
  ...
  const shareCode = String(newRows[0].share_code || '')
  ...
  try {
    ...
    if (!emailSent) {
      // Diagnóstico sem PII — o corpo pode conter detalhe do gateway/código;
      // só o status HTTP chega para distinguir 401 (segredo) de 429 (rate) de 5xx (Brevo).
      logInscriptionError('email_send_failed', { emailStatus: emailRes.status, slug: eventoSlug })
    }
  } catch (err) {
    logInscriptionError('email_send_network_error', {
      reason: err?.cause?.code || err?.name || 'unknown',
      slug: eventoSlug,
    })
  }
  ...
  return { success: true, inscriptionId, shareCode, emailSent }
}
