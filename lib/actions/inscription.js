...
export async function apiSubmitInscription(form, eventoId, eventoSlug) {
  ...
  if (!fnRes.ok) {
    // Mapear status → código semântico com base no que a Edge Function devolve.
    // 404 = event_not_found | 409 = event_full (validação distinta de duplicate)
    // 400 = validation_failed | 429 = rate_limited | resto = validation_failed
    let code = 'validation_failed'
    if (fnRes.status === 404) code = 'event_not_found'
    else if (fnRes.status === 409) {
      // A Edge Function devolve 409 também para duplicates (linha 207 do
      // validate-inscription/index.ts). A flag `validation.isDuplicate`
      // não existe — distinguir pela string da mensagem.
      const errMsg = String(validation?.error || '').toLowerCase()
      code = errMsg.includes('já está registado') || errMsg.includes('already') ? 'duplicate' : 'event_full'
    ...
    // Para 'duplicate' mantemos string legacy para backward-compat com clients
    // que ainda não fazem parse JSON (InscricaoPageClient.jsx já trata ambos).
    if (code === 'duplicate') throw new Error('duplicate')
    throwCode(code, validation?.error || null)
  }
