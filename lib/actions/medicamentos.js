...
export async function saveDrugProfile(drugId, data) {
  ...
  const { error } = await ctx.supabase
    .from("drug_profiles")
    .upsert({ drug_id: drugId, ...parsed.data }, { onConflict: "drug_id" });
...
// ============================================================
export async function getPublicDrugsWithInfo(lang = "pt") {
  const supabase = await createClient();
...
// ============================================================
export async function getPublicDrugBySlug(slug, lang = "pt") {
  const supabase = await createClient();
  ...
  const { data: profile } = await supabase
    .from("drug_profiles")
    .select(
      "overview_public_pt, overview_public_en, overview_pro_pt, overview_pro_en, " +
...
// ============================================================
export async function getPublicDrugInteractionsForDrug(drugId, lang = "pt") {
  const supabase = await createClient();
  ...
  return {
    ...
    pregnancy: (pregnancyRes.data || []).map((i) => ({
      id: i.id,
      drugId: i.drug_id,
