...
const VALID_MODES = ['deck', 'tipo', 'rapido']
 ...
 * servidor (answerQuiz).
 */
export async function startQuiz({ mode = 'rapido', deckSlug = '', source = 'mixed', count = 10 } = {}) {
  try {
 ...
 */
export async function finishQuiz({ mode = 'rapido', source = 'mixed', deckId = null, total = 0, correct = 0, details = [] } = {}) {
  try {
    const supabase = await createClient()
