...
import { getQuizPools, getQuizPoolCounts } from '@/lib/api/quiz'
...
/** Contagens reais dos pools (admin) — para detetar tipos escassos. */
export async function getQuizPoolCountsAdmin() {
  const ctx = await requireAdmin()
  if (!ctx) return null
  try {
    return await getQuizPoolCounts()
  } catch {
    return null
  }
}
