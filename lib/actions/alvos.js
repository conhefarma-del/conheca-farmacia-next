...
import { z } from 'zod'
import { splitList } from '@/lib/targets/derive'

// Palavras-reserva que não são fármacos (aparecem quando a lista está vazia
// ou em revisão, ex.: "Indutores (lista em revisão)." sem ':' → o splitList
// devolveria 'Indutores' como item).
...
/**
 * Nº de fármacos por papel (substrato/inibidor/indutor) de cada alvo,
 * contado DIRECTAMENTE das listas de texto (substrates_pt/inhibitors_pt/
 * inducers_pt) — reflete o conteúdo editorial do alvo, mesmo quando os
 * fármacos das listas ainda não existem na tabela drugs (não geram linhas
 * em drug_target_roles). Usa a mesma normalização de lib/targets/derive.js
 * para consistência. Usado nos cards da listagem /alvos.
 */
export const getTargetDrugCounts = unstable_cache(
  async () => {
    ...
    const { data, error } = await supabase
      .from('molecular_targets')
      .select('id, substrates_pt, inhibitors_pt, inducers_pt')
      .eq('status', 'published')
      .eq('is_archived', false)
    if (error) return {}
    const counts = {}
    ;(data || []).forEach((t) => {
      const real = (text) =>
        splitList(text).filter((x) => !COUNT_RESERVED.has(x.toLowerCase())).length
      counts[t.id] = {
        substrate: real(t.substrates_pt),
        inhibitor: real(t.inhibitors_pt),
        inducer: real(t.inducers_pt),
      }
    })
    return counts
  },
