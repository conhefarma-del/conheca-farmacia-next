...
// ============================================================
//  Mapa de papéis por fármaco — para cruzar pares nos cartões de interações
// ============================================================

/**
 * Todos os papéis publicados agrupados por fármaco: { [drugId]: [{ role,
 * target }] }. Usado nos cartões de interações fármaco-fármaco do perfil
 * para destacar quando o par é exatamente substrato + inibidor/indutor do
 * mesmo alvo (auto-interação do par, ex.: claritromicina × simvastatina).
 */
export const getTargetRolesByDrug = unstable_cache(
  async (lang = 'pt') => {
    const supabase = await createAnonClient()
    const { data, error } = await supabase
      .from('drug_target_roles')
      .select('drug_id, role, target:target_id(id, slug, name_pt, name_en)')
      .eq('status', 'published')
      .eq('is_archived', false)
    if (error) return {}
    const map = {}
    ;(data || []).forEach((r) => {
      if (!map[r.drug_id]) map[r.drug_id] = []
      map[r.drug_id].push({
        role: r.role,
        target: {
          id: r.target?.id,
