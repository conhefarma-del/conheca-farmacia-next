...
const guideDisciplineSchema = z.object({
  ...
  practice_pt: z.string().optional().default(''),
  practice_en: z.string().optional().default(''),
  learning_pt: z.string().optional().default(''),
  learning_en: z.string().optional().default(''),
  status: z.enum(['draft', 'published']).default('draft'),
  sort_order: z.number().int().optional().default(0),
})
...
const DISCIPLINE_COLS = 'id, slug, course_id, name_pt, name_en, description_pt, description_en, phase_pt, phase_en, importance_pt, importance_en, practice_pt, practice_en, learning_pt, learning_en, sort_order'
 ...
 */
export const getPublicGuideCourses = unstable_cache(
  async (lang = 'pt') => {
 ...
 */
export const getPublicGuideCourseBySlug = unstable_cache(
  async (slug, lang = 'pt') => {
  const supabase = await createAnonClient()
  try {
    ...
    const disciplinesResolved = await Promise.all((disciplines || []).map(async (d) => {
      ...
      return {
        ...
        practice: pickLang(d, 'practice', lang),
        learning: pickLang(d, 'learning', lang),
