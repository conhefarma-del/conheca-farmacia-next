...
// Schema de lista de classes terapêuticas (CollectionPage + ItemList).
// Sinal E-E-A-T para catálogo de classes de fármacos: cada item liga à
// sua página de detalhe, facilita a navegação e fornece dados estruturados
// a motores de busca sobre a organização do conteúdo farmacêutico.
export function buildClassesListSchema(classes, lang = 'pt') {
  if (!classes || classes.length === 0) return null

  const pageUrl = `${SITE_URL}/${lang}/classes`

  return [
    {
      '@context': 'https://schema.org',
      '@type': 'CollectionPage',
      'name': lang === 'en' ? 'Drug Classes' : 'Classes Terapêuticas',
      'description':
        lang === 'en'
          ? 'Browse all therapeutic drug classes with descriptions, ATC codes and related medicines.'
          : 'Explore todas as classes terapêuticas de fármacos com descrições, códigos ATC e medicamentos relacionados.',
      'url': pageUrl,
      'inLanguage': lang === 'en' ? 'en' : 'pt',
      'isPartOf': {
        '@type': 'WebSite',
    ...
    },
    {
      ...
      'itemListElement': classes.map((cls, i) => ({
        ...
        'item': {
          ...
          'url': `${SITE_URL}/${lang}/classes/${cls.slug}`,
          'about': {
            '@type': 'MedicalEntity',
            'name': cls.name,
            ...(cls.atcPrefix
