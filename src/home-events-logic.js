...
function formatDateCard(dateStr) {
	...
	return { day, month, full: date.toLocaleDateString('pt-PT', { day: 'numeric', month: 'long', year: 'numeric' }) };
}
...
function renderEventsGrid(events, container) {
	...
	if (events.length === 0) {
		container.innerHTML = '<p class="text-center text-brand-deep/60">Nenhum evento disponível no momento.</p>';
		return;
	}
	...
	events.slice(0, 2).forEach(event => {
		...
		const fullDate = dateObj.toLocaleDateString('pt-PT', { day: 'numeric', month: 'long', year: 'numeric' });
...
function renderLivesGrid(lives, container) {
	...
	if (lives.length === 0) {
		container.innerHTML = '<p class="text-center text-brand-deep/60">Nenhuma live disponível no momento.</p>';
		return;
	}
