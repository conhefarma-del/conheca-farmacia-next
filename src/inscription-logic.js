...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function validateFormData(data) {
    ...
    if (
      !data.telefone ||
      (!phoneRegex.test(phoneClean) && !intlRegex.test(phoneClean))
    ) {
      errors.push(
        "Telefone inválido. Use formato: +244 925 696 002 ou 925 696 002"
      );
    ...
    if (!data.evento_slug || data.evento_slug.trim() === "") {
      errors.push(
        "Evento não especificado. Por favor, selecione um evento válido"
      );
  ...
  async function validateWithEdgeFunction(data, honeypot) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          result.error || "Erro ao validar inscrição no servidor"
        );
      }
      ...
      if (error.name === "AbortError") {
        throw new Error(
          "A ligação ao servidor demorou demasiado tempo. Por favor, tente novamente."
        );
      }
      ...
      if (error instanceof TypeError) {
        ...
        throw new Error(
          "Não foi possível contactar o servidor. Por favor, verifique a sua ligação ou tente mais tarde."
        );
      }
  ...
  inscriptionForm.addEventListener("submit", async (e) => {
    ...
    if (now - lastSubmissionTime < RATE_LIMIT_WINDOW) {
      const remainingTime = Math.ceil(
        (RATE_LIMIT_WINDOW - (now - lastSubmissionTime)) / 1000
      );
      showError(
        `Por favor, aguarde ${remainingTime} segundo(s) antes de tentar novamente.`
      );
      return;
    }
    ...
    try {
      ...
      if (isDuplicate) {
        throw new Error(
          "Já tem uma inscrição neste evento com este endereço de email. Se tem dúvidas, contacte-nos."
        );
      }
      ...
      if (error) {
        ...
        if (error.code === "42501") {
          console.error(
            "ℹ️ ERRO RLS: Verifique os dados contra a política de segurança"
          );
          throw new Error(
            "A inscrição foi rejeitada pelas políticas de segurança. Verifique se preencheu todos os campos corretamente."
          );
        }
      ...
      showError(
        error.message ||
          "Não foi possível completar a sua inscrição neste momento. Por favor, tente novamente mais tarde ou contacte o suporte."
      );
