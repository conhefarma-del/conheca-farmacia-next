...
export async function generateMetadata({ params }) {
  ...
  // Both /pt/artigos and /en/articles are valid URL paths for the same content.
  ...
  if (safeLang === 'en' && article.hasOwnProperty('article_id')) {
    ...
    try {
      ...
      if (enTr) langs['en'] = `/en/articles/${enTr.slug}`
    } catch { /* leave out */ }
  }

  const articleUrl = `${SITE_URL}/${safeLang}/articles/${article.slug}`
...
export default async function ArticleDetailPage({ params }) {
  ...
  // Breadcrumb levels — use /articles path for EN, /artigos for PT
  const articlesListHref = safeLang === 'en' ? `/${safeLang}/articles` : `/${safeLang}/artigos`
  const breadcrumbLevels = [
    { label: tFn('nav.inicio'), href: `/${safeLang}` },
    { label: tFn('nav.artigos'), href: articlesListHref },
    { label: article.title },
  ]
