...
export default function EntrarClient({ lang }) {
  ...
  const handleEmailLogin = async (e) => {
    ...
    try {
      ...
      if (signInError) {
        setError(signInError.message || 'Email ou password incorretos')
        setLoading(false)
        return
      }
      ...
      if (data.user) {
        ...
        window.location.href = `/${lang}/perfil`
      }
    } catch {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (oauthError) {
        setError(oauthError.message || 'Erro ao iniciar sessão com Google')
        setLoading(false)
      }
    } catch {
      setError('Erro ao conectar ao servidor')
    ...
    <section className="py-20 bg-background">
      <div className="container-center max-w-md mx-auto px-4">
        <div className="text-center mb-8">
          <LogIn size={40} className="mx-auto mb-4 text-brand-accent" />
          <h1 className="text-3xl font-bold text-brand-deep mb-2">Entrar</h1>
          <p className="text-brand-deep/60">Acede à tua conta Conheça Farmácia</p>
        </div>
        ...
        <p className="text-center text-sm text-brand-deep/60 mt-6">
          Não tens conta?{' '}
          <Link href={`/${lang}/registo`} className="text-brand-accent hover:underline">
            Criar conta
          </Link>
        </p>
      </div>
