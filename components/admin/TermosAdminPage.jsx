...
import { createTermsSection, updateTermsSection, archiveTermsSection, restoreTermsSection } from '@/lib/actions/legalContent'

export default function TermosAdminPage({ lang, initialSections, currentUserRole }) {
  ...
  const handleSaveSection = useCallback(async () => {
    ...
    try {
      if (panelMode === 'create') {
        const result = await createTermsSection({
        ...
        const result = await updateTermsSection(editingSection.id, {
          anchor_slug: formAnchorSlug,
          title_pt: formTitlePt,
          title_en: formTitleEn,
          content_pt: formContentPt,
  ...
  const handleArchive = useCallback(async (id) => {
    const result = await archiveTermsSection(id)
    if (result.success) {
  ...
  const handleRestore = useCallback(async (id) => {
    const result = await restoreTermsSection(id)
    if (result.success) {
  ...
  return (
    <div className="admin-terms">
      <div className="admin-page-header">
        <h1>Gerir Termos de Utilização</h1>
        <p className="admin-page-subtitle">Gerir secções dos termos de utilização do website.</p>
      </div>
      ...
      <div className="admin-card">
        ...
        <div className="admin-card-body">
          {/* Wrapper com scroll horizontal: em mobile a tabela é mais larga que
              o container e sem overflow-x transbordaria a página (convenção
              GuidesAdminPage / PrivacyAdminPage). */}
            ...
            >
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <input
                  ...
                  onFocus={(e) => { e.target.style.borderColor = '#00493a'; e.target.style.boxShadow = '0 0 0 3px rgba(0,73,58,0.1)' }}
                  onBlur={(e) => { e.target.style.borderColor = '#d1d5db'; e.target.style.boxShadow = 'none' }}
                  placeholder="Ex: Aceitação dos Termos"
                />
              </div>
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <input
                  ...
                  onFocus={(e) => { e.target.style.borderColor = '#00493a'; e.target.style.boxShadow = '0 0 0 3px rgba(0,73,58,0.1)' }}
                  onBlur={(e) => { e.target.style.borderColor = '#d1d5db'; e.target.style.boxShadow = 'none' }}
                  placeholder="Ex: Acceptance of Terms"
                />
              </div>
      ...
      <style>{`
        .admin-terms .admin-table th,
        .admin-terms .admin-table td {
          padding: 10px 12px;
          text-align: left;
        ...
        .admin-terms .admin-table-row-child td {
          font-size: 13px;
        }
        ...
        @media (max-width: 768px) {
          .admin-terms .admin-table-wrapper {
            border: none;
            border-radius: 12px;
          }
          .admin-terms .admin-table-actions {
            flex-wrap: nowrap;
            flex-direction: column;
            align-items: stretch;
          }
          .admin-terms .admin-table-actions .admin-btn-sm {
