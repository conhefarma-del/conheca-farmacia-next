...
- ⚠ **Avisa (não bloqueia)**: uso dinâmico no corpo do page component — o Next captura e serve a rota dinâmica (perde ISR silenciosamente, mas não dá 500). Estado actual: **zero avisos** — as 3 páginas legais (`faq`, `politica-privacidade`, `termos`) foram corrigidas em 2026-09-18: `getPublic*Data()` em `lib/actions/legalContent.js` passou para `createAnonClient()` (seguro porque as policies `anon_read_*` das migrações 034/035/247 já expõem `is_archived = false` ao role anon — o client com cookies usava exactamente esse role em visitantes). As funções admin (via `requireAdmin()`) mantêm o client com cookies.
